...
MeshView::MeshView(HINSTANCE hInstance)
    : D3DApp(hInstance)
    , mSky(0), mTreeModel(0), mBaseModel(0)
    , mStairsModel(0), mPillar1Model(0), mPillar2Model(0)
    , mPillar3Model(0), mPillar4Model(0), mRockModel(0)
    , mScreenQuadVB(0), mScreenQuadIB(0)
    , mSmap(0), mSsao(0), mLightRotationAngle(0)
{
    mMainWndCaption = L"MeshView";
...
void MeshView::UpdateScene(float dt)
{
...
void MeshView::OnMouseDown(WPARAM btnState, int x, int y)
{
    mLastMousePos.x = x;
    mLastMousePos.y = y;
...
void MeshView::OnMouseUp(WPARAM btnState, int x, int y)
{
    ReleaseCapture();
}

void MeshView::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void MeshView::DrawSceneToSsaoNormalDepthMap()
{
    XMMATRIX view = mCam.View();
    XMMATRIX proj = mCam.Proj();
...
void MeshView::DrawSceneToShadowMap()
{
...
void MeshView::DrawScreenQuad(ID3D11ShaderResourceView* srv)
{
...
void MeshView::BuildShadowTransform()
{
...
void MeshView::BuildScreenQuadGeometryBuffers()
{
