...
Shadows::Shadows(HINSTANCE hInstance)
    : D3DApp(hInstance)
    , mSky(0)
    ...
    , mSmap(0), mLightRotationAngle(0.0f)
{
...
bool Shadows::Init()
{
    ...
    ScratchImage brick;
    HR(LoadFromDDSFile(L"textures/bricks.dds", DDS_FLAGS_NONE, nullptr, brick));
    HR(CreateShaderResourceView(md3dDevice, brick.GetImages(), brick.GetImageCount(), brick.GetMetadata(), &mBrickTexSRV));

    ScratchImage floorNormal;
    HR(LoadFromDDSFile(L"textures/floor_nmap.dds", DDS_FLAGS_NONE, nullptr, floorNormal));
    ...
    ScratchImage brickNormal;
    HR(LoadFromDDSFile(L"textures/bricks_nmap.dds", DDS_FLAGS_NONE, nullptr, brickNormal));
    HR(CreateShaderResourceView(md3dDevice, brickNormal.GetImages(), brickNormal.GetImageCount(), brickNormal.GetMetadata(), &mBrickNormalTexSRV));
...
void Shadows::UpdateScene(float dt)
{
...
void Shadows::OnMouseDown(WPARAM btnState, int x, int y)
{
    mLastMousePos.x = x;
    mLastMousePos.y = y;
...
void Shadows::OnMouseUp(WPARAM btnState, int x, int y)
{
    ReleaseCapture();
}

void Shadows::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void Shadows::DrawSceneToShadowMap()
{
...
void Shadows::BuildShapeGeometryBuffers()
{
...
void Shadows::BuildSkullGeometryBuffers()
{
...
void Shadows::BuildScreenQuadGeometryBuffers()
{
