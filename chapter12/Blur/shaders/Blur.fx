...
void HorzBlurCS(int3 groupThreadID : SV_GroupThreadID,
                int3 dispathThreadID : SV_DispatchThreadID)
{
    ...
    float4 blureColor = float4(0, 0, 0, 0);
    
    [unroll]
    for (int i = -gBlurRadius; i <= gBlurRadius; ++i)
    {
        int k = groupThreadID.x + gBlurRadius + i;
        blureColor += gWeights[i + gBlurRadius] * gCache[k];
    }
    
    gOutput[dispathThreadID.xy] = blureColor;
}
